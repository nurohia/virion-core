...
add_direct_forward() {
    ...
    tmp_file=$(mktemp)
    yq eval ".inbounds += [{type: \"tcp\", listen: \"0.0.0.0:$listen_port\"}]" "$RULE_FILE" > "$tmp_file"
    mv "$tmp_file" "$RULE_FILE"

    tmp_file=$(mktemp)
    yq eval ".outbounds += [{type: \"tcp\", remote: \"$target\"}]" "$RULE_FILE" > "$tmp_file"
    mv "$tmp_file" "$RULE_FILE"
...
add_relay_forward() {
    ...
    tmp_file=$(mktemp)
    yq eval ".inbounds += [{type: \"tcp\", listen: \"0.0.0.0:$entry_port\"}]" "$RULE_FILE" > "$tmp_file"
    mv "$tmp_file" "$RULE_FILE"

    tmp_file=$(mktemp)
    yq eval ".outbounds += [{type: \"tcp\", remote: \"$middle_target\"}]" "$RULE_FILE" > "$tmp_file"
    mv "$tmp_file" "$RULE_FILE"
...
main_menu() {
    ...
    case $choice in
        ...
        5) nohup virion -c "$RULE_FILE" >> "$LOG_FILE" 2>&1 & echo "[✓] 已启动 Virion...";;
